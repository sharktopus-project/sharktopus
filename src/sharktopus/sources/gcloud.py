...
def fetch_step(
    ...
    *,
    ...
    When *variables* and *levels* are both provided, the function
    switches to **byte-range mode**: it fetches the ``.idx`` file,
    filters records by ``VAR:LEVEL``, and downloads only those byte
    ranges in parallel (typically ~15 MB instead of ~500 MB). Omit
    both to fall back to the full-file + local-crop recipe.
    """
